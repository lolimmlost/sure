    ...
    def transactions_only_query_sql
      <<~SQL
        SELECT
          ...
          CASE WHEN at.kind IN ('investment_contribution', 'loan_payment', 'savings_contribution') THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END as classification,
          ABS(SUM(CASE WHEN at.kind IN ('investment_contribution', 'loan_payment', 'savings_contribution') THEN ABS(ae.amount * COALESCE(er.rate, 1)) ELSE ae.amount * COALESCE(er.rate, 1) END)) as total,
        ...
        GROUP BY c.id, c.parent_id, CASE WHEN at.kind IN ('investment_contribution', 'loan_payment', 'savings_contribution') THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END;
      SQL
    end
