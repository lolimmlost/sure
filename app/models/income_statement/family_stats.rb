    ...
    def query_sql
      <<~SQL
        WITH period_totals AS (
          ...
          GROUP BY period, CASE WHEN t.kind IN ('investment_contribution', 'loan_payment', 'savings_contribution') THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END
        )
        SELECT
          classification,
